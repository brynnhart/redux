...
function initDB() {
  ...
  if (itemCount === 0) {
    ...
    const weapons = [
      { name: 'Stick',         description: 'A sturdy walking stick repurposed for violence.',   gold_value: 200,       modifiers: JSON.stringify({ attack: 2   }) },
      { name: 'Dagger',        description: 'Quick and light; favored by those who value speed.', gold_value: 1000,      modifiers: JSON.stringify({ attack: 5  }) },
      { name: 'Short Sword',   description: 'Reliable and easy to carry.',                        gold_value: 3000,      modifiers: JSON.stringify({ attack: 10  }) },
      { name: 'Long Sword',    description: 'The weapon of the common soldier.',                  gold_value: 10000,     modifiers: JSON.stringify({ attack: 15  }) },
      { name: 'Huge Axe',      description: 'Devastating in strong hands.',                       gold_value: 30000,     modifiers: JSON.stringify({ attack: 20  }) },
      { name: 'Bone Cruncher', description: 'Its name explains everything you need to know.',    gold_value: 100000,    modifiers: JSON.stringify({ attack: 30  }) },
      { name: 'Twin Swords',   description: 'Two blades, twice the carnage.',                     gold_value: 150000,    modifiers: JSON.stringify({ attack: 40  }) },
      { name: 'Power Axe',     description: 'Enchanted steel that hums with fury.',               gold_value: 200000,    modifiers: JSON.stringify({ attack: 60 }) },
      { name: "Able's Sword",  description: "Forged by the legendary smith Able.",                gold_value: 400000,    modifiers: JSON.stringify({ attack: 90 }) },
      { name: "Wan's Weapon",  description: 'Its origin is a mystery even to its owner.',         gold_value: 1000000,   modifiers: JSON.stringify({ attack: 125 }) },
      { name: 'Spear of Gold', description: 'A golden spear blessed by forgotten gods.',          gold_value: 4000000,   modifiers: JSON.stringify({ attack: 175 }) },
      { name: 'Crystal Shard', description: 'A razor sliver of pure crystallized power.',         gold_value: 10000000,  modifiers: JSON.stringify({ attack: 250 }) },
      { name: "Nira's Teeth",  description: 'Nobody asks what Nira was.',                         gold_value: 40000000,  modifiers: JSON.stringify({ attack: 400 }) },
      { name: 'Blood Sword',   description: 'It drinks what it spills.',                          gold_value: 100000000, modifiers: JSON.stringify({ attack: 600}) },
      { name: 'Death Sword',   description: 'The last weapon you will ever need.',                gold_value: 400000000, modifiers: JSON.stringify({ attack: 1000}) },
    ];

    // ── Armour (defense modifiers) — 16 tiers ─────────────────────────────
    const armours = [
      { name: 'Coat',             description: 'Better than nothing. Barely.',                    gold_value: 200,       modifiers: JSON.stringify({ defense: 2   }) },
      { name: 'Heavy Coat',       description: 'Thick wool with some toughened leather panels.',  gold_value: 1000,      modifiers: JSON.stringify({ defense: 5  }) },
      { name: 'Leather Vest',     description: 'Supple, quiet, and better than cloth.',           gold_value: 3000,      modifiers: JSON.stringify({ defense: 10  }) },
      { name: 'Bronze Armour',    description: 'The first step toward real protection.',          gold_value: 10000,     modifiers: JSON.stringify({ defense: 15  }) },
      { name: 'Iron Armour',      description: 'Heavy, reliable, and completely unfashionable.',  gold_value: 30000,     modifiers: JSON.stringify({ defense: 20  }) },
      { name: 'Graphite Armour',  description: 'Lightweight and surprisingly strong.',            gold_value: 100000,    modifiers: JSON.stringify({ defense: 30  }) },
      { name: "Erdrick's Armour", description: 'Once worn by the hero Erdrick himself.',          gold_value: 150000,    modifiers: JSON.stringify({ defense: 40  }) },
      { name: 'Armour of Death',  description: 'It was taken from someone who no longer needed it.',gold_value: 200000,  modifiers: JSON.stringify({ defense: 60 }) },
      { name: "Able's Armour",    description: 'A matched set with the famous sword.',            gold_value: 400000,    modifiers: JSON.stringify({ defense: 90 }) },
      { name: 'Full Body Armour', description: 'Head to toe in forged steel.',                    gold_value: 1000000,   modifiers: JSON.stringify({ defense: 125 }) },
      { name: 'Blood Armour',     description: 'Stained crimson. Do not ask how.',                gold_value: 4000000,   modifiers: JSON.stringify({ defense: 175 }) },
      { name: 'Magic Protection', description: 'Woven with spells older than the kingdom.',       gold_value: 10000000,  modifiers: JSON.stringify({ defense: 250 }) },
      { name: "Belar's Mail",     description: 'Belar was never defeated in battle.',             gold_value: 40000000,  modifiers: JSON.stringify({ defense: 400 }) },
      { name: 'Golden Armour',    description: 'Blinding to behold, impenetrable to strike.',    gold_value: 100000000, modifiers: JSON.stringify({ defense: 600}) },
      { name: 'Armour of Lore',   description: 'Ancient armour said to carry the knowledge of ages.',gold_value: 400000000,modifiers: JSON.stringify({ defense: 1000}) },
    ];
