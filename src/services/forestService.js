...
export class ForestService {
    ...
    eventService;
    rng;
    constructor(playerRepo, newsService, stateRepo = new ForestStateRepo(), combatService = new CombatService(), eventService = new ForestEventService(), rng = Math.random) {
    ...
    look(player, today) {
        ...
        if (this.rng() < 0.7) {
            ...
            this.stateRepo.upsertEncounter(player.id, 'ENEMY', enemy.key, enemy);
    ...
    attack(player, today) {
        ...
        const enemy = current.encounterPayload;
    ...
    useSkill(player, today, skillKey) {
        const current = this.stateRepo.findByPlayerId(player.id);
        if (current.encounterType !== 'ENEMY') {
    ...
    searchDragon(player, today) {
        ...
        const rounds = ['You track scorched footprints and sulfur to a cavern of bones.'];
        ...
        try {
            if (dragonHp <= 0) {
                ...
                db.prepare(`INSERT INTO player_history (player_id, day_key, event_type, lap_before, lap_after, created_at)
           VALUES (?, ?, 'dragon_kill', ?, ?, ?)`).run([player.id, today, lapBefore, lapAfter, new Date().toISOString()]);
                db.exec('COMMIT');
                return {
