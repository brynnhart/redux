...
function isDayKeyBefore(left, right) {
    if (!left) {
        return true;
    }
    return left < right;
...
export class DayService {
    ...
    newsService;
    rng;
    constructor(playerRepo = new PlayerRepo(), newsService = new NewsService(), rng = Math.random) {
        this.playerRepo = playerRepo;
        this.newsService = newsService;
        this.rng = rng;
    }
    ensureDailyReset(playerId) {
    ...
    rolloverPlayerToNewDay(playerId, todayDayKey) {
        const db = getDb();
        const player = this.playerRepo.findById(playerId);
        if (!player) {
            throw new Error('Player not found');
    ...
    mapPendingEventsToNews(events) {
        return events.map((event) => {
            ...
            if (event.type === 'pvp_killed') {
                ...
                return { message: `${killerName} has killed ${victimName}.`, severity: 'pvp', playerId: null };
            }
            ...
            return { message: `The Red Dragon has killed ${victimName}!`, severity: 'dragon', playerId: null };
        });
    }
