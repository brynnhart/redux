...
import type { PlayerClass, PlayerRecord } from '../repos/playerRepo.js';

export type SkillPath = 'DEATH' | 'MYSTIC' | 'THIEF';

export function getSkillPath(klass: PlayerClass): SkillPath {
  if (klass === 'DEATH_KNIGHT') return 'DEATH';
  if (klass === 'MYSTICAL') return 'MYSTIC';
  return 'THIEF';
}

export function getDailySkillUses(level: number, hasMastery: boolean): number {
  const base = 1 + Math.floor(level / 3) + (hasMastery ? 1 : 0);
  return Math.min(config.skillDailyUsesCap, Math.max(1, base));
}

export function classLabel(klass: PlayerClass) {
  if (klass === 'DEATH_KNIGHT') return 'Death Knight';
  if (klass === 'MYSTICAL') return 'Mystical Student';
  return 'Thieving Apprentice';
}

export function skillLabel(klass: PlayerClass) {
  if (klass === 'DEATH_KNIGHT') return 'Death Skill';
  if (klass === 'MYSTICAL') return 'Mystic Skill';
  return 'Thief Skill';
}

export function getClassSkillLevel(player: PlayerRecord): number {
  if (player.class === 'DEATH_KNIGHT') return player.skill_level_death;
  if (player.class === 'MYSTICAL') return player.skill_level_mystic;
  return player.skill_level_thief;
}

export function getClassSkillUses(player: PlayerRecord): number {
  if (player.class === 'DEATH_KNIGHT') return player.skill_uses_death;
  if (player.class === 'MYSTICAL') return player.skill_uses_mystic;
  return player.skill_uses_thief;
}

export function consumeClassSkillUsePatch(player: PlayerRecord): Partial<PlayerRecord> {
