...
import type { ScreenState, Session } from '../session.js';
...
export function isNavigationHelperScreen(state: ScreenState): boolean {
  return Boolean(screens[state]);
}

export function renderNavigationHelperScreen(session: Session, dims: Dimensions) {
...
export function handleNavigationHelperInput(session: Session, inputText: string): Transition {
  const screen = screens[session.state];
  if (!screen) {
    return { type: 'error', message: 'Huh?' };
  }
